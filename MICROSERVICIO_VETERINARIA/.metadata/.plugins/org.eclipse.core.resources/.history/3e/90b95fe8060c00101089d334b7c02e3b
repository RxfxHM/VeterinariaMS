...
import com.mx.Mascotas.DTO.MascotaDetalle;
...
public class MascotasWS {
	...
	public ResponseEntity<?> detalleMascota(@RequestBody Mascotas mascota) {
		...
		Veterinaria veterinaria = service.findVeterinaria(mascota);
		if (responsable == null && cliente == null && veterinaria == null) {
			return ResponseEntity.status(HttpStatus.NO_CONTENT).build();
		} else {
			MascotaDetalle response = new MascotaDetalle(responsable, cliente, veterinaria);
